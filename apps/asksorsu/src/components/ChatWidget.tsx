...
import { MessageCircle, X, RotateCcw, Send, AlertTriangle, Maximize2, Minimize2 } from 'lucide-react'
...
export default function ChatWidget() {
  ...
  // Panel sizing: fullscreen fills the viewport; windowed is the floating card
  const panelClass = isFullscreen
    ? 'fixed inset-0 z-50 flex flex-col overflow-hidden'
    : 'fixed bottom-24 right-4 sm:right-6 z-50 flex flex-col rounded-2xl shadow-2xl overflow-hidden'
  ...
  return (
    <>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={isFullscreen ? 'max-w-[600px] w-full' : 'max-w-[82%]'}>
                  <div
      ...
      {/* Floating toggle button — hidden when fullscreen so it doesn't overlap */}
      {!isFullscreen && (
        <button
