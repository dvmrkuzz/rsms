...
export default function AnnouncementsPage() {
  ...
  const [form, setForm] = useState({
    title: '', content: '', target: 'all', expiresAt: '',
  })
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Post and manage campus announcements</p>
        </div>
        <button
        ...
        {isLoading ? (
          ...
          data?.data?.map((a: Announcement) => (
            <div key={a.id} className={`bg-white rounded-xl border shadow-sm p-5 ${
              !a.isActive ? 'opacity-60' : 'border-gray-100'
            }`}>
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6">
            <h3 className="font-semibold text-gray-800 mb-5">New Announcement</h3>
